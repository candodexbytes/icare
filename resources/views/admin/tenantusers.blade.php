...
<div id="page-wrapper" class="tenantusers" style="min-height: 140px;">    
    <div class="col-sm-12 col-md-12 graphs mrg-top clearfix">    
        <div class="col-lg-12 no-pad">

            <h1 class="heading_text text-right"><span class="pull-left">Tenant Users</span>@if($type == 5)
                <!--<a href="{{url('admin/new-tenant-user')}}"><span class="btn btn-default my_btn"><i class="fa fa-plus"></i></span></a>-->
                @else <br/> @endif
            </h1>
        </div>
       <div class="clearfix">
        ...
        <div class="main-div">
            <table id="user_list" class="display nowrap" cellspacing="0" width="100%">
                ...
                <tbody id="the_list">
                    ...
                    <tr>
                        @if($type == 5  || $type == 6 || $type == 0)
                           <td class="detail_unit" data-user_id="{{$value->id}}"  data-ptd_id="{{str_replace(' ','-',$value->ptd_id)}}"></td>
                          @else
                        ...
                        <td class="details-control btn-link" data-ptd_id="{{$genrate_ptd_id}}" data-user_id="{{$value->id}}" >
<!--                            <a href="{{url('/')}}/admin/my-family/{{$genrate_ptd_id}}/{{$value->id}}">View</a>-->
                               View
                        </td>                                       
                        ...
                        @if($type == 5)
                        <td>
                            <span  data-name="{{$value->name}}" data-nric="{{$value->nric}}" data-email="{{$value->email}}" data-ptdid="{{$genrate_ptd_id}}" data-id="{{$value->id}}" data-mobilenumber="{{$value->mobile_number}}"  class="edit_btn"><i class="fa fa-pencil" aria-hidden="true"></i></span>                 
                        </td>
                        @endif
                    </tr>
                    @endforeach
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header middle">
                ...
                <h4 class="modal-title">Edit <span id="title_set">Tenant Users</span> </h4>
            </div>
            <div class="modal-body pad">
                <form method="post" action="{{action('AdminController@UpdateTenantUser')}}"  class="conatct-form form-horizontal" id="update_tenantuser_form" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <label class="col-sm-2 control-label" for="nric"><strong>NRIC</strong></label>
                        <div class="col-sm-9">
                    ...
                    <div id="pwd_section">
                    <div class="form-group">
                        <label class="col-sm-2 control-label" for="password"><strong>Password</strong></label>
                        <div class="col-sm-9">
                            <input class="form-control" type="password"  name="password" id="password"  />
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label" for="confirm_password"><strong>Confirm Password</strong></label>
                        <div class="col-sm-9">
        ...
        user_id=$(this).data('user_id');  
          ...
          if ( row.child.isShown() ) {
            ...
            tr.removeClass('shown');
             ...
             if(ptd_id!=="" && user_id!==""){
          var unit_user_url = '{{url('admin/get-user-unit')}}/' +ptd_id+'/'+ user_id; 
             $.ajax({
            type: "GET",
...
$('#pwd_section').hide();
    ...
    function getTenantdetails(genrate_ptd_id, id){
    window.location.href = "{{url('/')}}/admin/my-family/" + genrate_ptd_id + "/" + id;
    }
</script>
@endsection
@endguest
